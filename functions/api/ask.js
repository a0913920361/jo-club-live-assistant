...
活動與現場服務：
- 店員配置：店長小江，外號阿鼠，負責現場統籌、活動規劃與客人特殊需求判斷；小邵固定負責廚房出餐。
- 團隊曾經營德州撲克協會，擅長活動節奏、氛圍營造、社交引導與情緒價值。
- 二樓屬於私人熟客場域，有麻將桌、德州撲克桌、網咖電腦 6 台。這不是公開必選項，不要主動推銷；若客人問私人安排，請保守回覆「可由專員評估是否適合安排」。

轉專員規則：
- 遇到生日、包場、活動企劃、特殊時段、私人場域、多人聚會、設備需求、超出菜單或優惠表能判斷的內容，要建議聯絡店長小江協助確認。
    ...
    for (const model of models) {
      try {
        ...
        return json({
          ok: true,
          model,
          answer: answer || "我目前沒有足夠資訊判斷，建議直接在官方 LINE 留言，讓店長小江協助確認。"
        });
      } catch (error) {
        failures.push({
          model,
